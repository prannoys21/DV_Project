...
function makeLineChart(dataset, xName, yObjs, axisLables) {
    ...
    chartObj.MxAxisLable = axisLables.MxAxis;
    chartObj.MyAxisLable = axisLables.MyAxis;
    /*
     yObjsects format:
...
/*var iata;
//= selectedCircle.__data__["Airport IATA"] ;
    if (selectedCircle ==null)
        iata="PHX"; 
    else 
        iata= selectedCircle.__data__["Airport IATA"] ;
    if (( iata == dataset.variableA) && (curr_Year == dataset.variableI)) {
    
  */  
    chartObj.data = dataset;
    chartObj.margin = {top: 15, right: 60, bottom: 35, left: 50};
    chartObj.width = 650 - chartObj.margin.left - chartObj.margin.right;
    chartObj.height = 460 - chartObj.margin.top - chartObj.margin.bottom;
...
//Create axis
    chartObj.MxAxis = d3.svg.axis().scale(chartObj.xScale).orient("bottom").tickFormat(chartObj.xFormatter); //< Can be overridden in definition

    chartObj.MyAxis = d3.svg.axis().scale(chartObj.yScale).orient("left").tickFormat(chartObj.yFormatter); //< Can be overridden in definition
...
// Change chart size according to window size
    chartObj.update_svg_size = function () {
        ...
        chartObj.svg.select('.x.axis').attr("transform", "translate(0," + chartObj.height + ")").call(chartObj.MxAxis);
        chartObj.svg.select('.x.axis .label').attr("x", chartObj.width / 2);

        chartObj.svg.select('.y.axis').call(chartObj.MyAxis);
...
// Render the chart
    chartObj.render = function () {
        ...
        // Draw Axis
        chartObj.svg.append("g").attr("class", "x axis").attr("transform", "translate(0," + chartObj.height + ")").call(chartObj.MxAxis).append("text").attr("class", "label").attr("x", chartObj.width / 2).attr("y", 30).style("text-anchor", "middle").text(chartObj.MxAxisLable);

        chartObj.svg.append("g").attr("class", "y axis").call(chartObj.MyAxis).append("text").attr("class", "label").attr("transform", "rotate(-90)").attr("y", -42).attr("x", -chartObj.height / 2).attr("dy", ".71em").style("text-anchor", "middle").text(chartObj.MyAxisLable);
